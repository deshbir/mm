...
com.compro.ppt.Pick = function(){
		...
		function renderPick(pickRatio,obj){
			...
			if(pickRatio){
				if(pickRatio.x!=1||pickRatio.y!=1){
					...
					var scale_y_compensation = (attrs.size.y/2)*(pickRatio.y-1);
		...
		PickConstr.prototype.setProperties  = function(setIndex, attributes) {
			...
			this.updateHandles();
			Utils.fireEvent(this,this.events.STATE_CHANGED);
			if(this.instance[setIndex].type=='text'){
		...
		PickConstr.prototype.externalObject = function(){
			var self = this;
			return {
				rotate:function(degree){return self.rotate(degree);},
				resize:function(scale_x,scale_y){return self.resize(scale_x,scale_y);},
				deletePick:function(){return self.deletePick();},
				setProperties:function(setIndex, attributes){return self.setProperties(setIndex, attributes);},
